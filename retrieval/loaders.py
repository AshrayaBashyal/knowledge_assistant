...
def load_content_text(obj) -> list[LCDocument]:
    ...
    A Document points to an actual file on disk with specific parsing rules (like page-by-page loading for PDFs), while a Note is just a text field already sitting in memory. Since the underlying mechanics are fundamentally different, it makes sense to use explicit code branches.
    
    We explicitly check the concrete model type of Document(using isinstance instead of duck typing) because *how* to extract
    its text genuinely differs by mechanism. Everything else (Note, Memory, and any future simple-text content type) is duck-typed on `.content` instead of getting its own isinstance branch
