...
def load_document_text(document: Document) -> list[LCDocument]:
    ...
    PDFs are loaded page-by-page via LangChain's PyPDFLoader, so each
    resulting chunk can later cite a page number. Markdown/TXT don't have
    an equivalent page boundary, so they're loaded as a single Document
    and rely on chunk_index (added during splitting) for citations
    instead.
    """
